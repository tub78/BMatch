...
public:
    ...
    TPoolEnum ReadAttribute(goblinRootObject&,goblinImport&) throw();
    void ReadPool(goblinRootObject&,goblinImport&) throw();

    /// \brief Write an attribute to s file
    void WriteAttribute(const goblinRootObject&,goblinExport&,attributeBase*,TBaseType,const char*) const throw();
    void WriteAttribute(const goblinRootObject&,goblinExport& F,TPoolEnum token) const throw();

    /// \brief Write all pool data to file
    void WritePool(const goblinRootObject&,goblinExport&,const char*) const throw();
    ...
    TAttributeType attributeType,void* _value_ptr) throw()
{
    ...
    if (!_value_ptr) _value_ptr = UndefinedVoidPtr(table[_token].arrayType);
