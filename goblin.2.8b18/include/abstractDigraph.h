...
friend class supersaturatedNodes;
friend class deficientNodes;
...
public:
    ...
    virtual TFloat  Flow(TArc a) const throw(ERRange) {return Sub(a);};
    ...
    TNode  MinimumMeanCycle(TOptSPX characteristic,TFloat* meanValue) throw();
    ...
    TNode  DAGSearch(TOptDAGSearch opt,TNode s=NoNode,TNode t=NoNode) throw(ERRange);
...
private:
    ...
    TFloat  MXF_PushRelabel(TNode,TNode) throw(ERRange);
...
public:
    ...
    supersaturatedNodes(const abstractDiGraph& _G) throw() :
    ...
    bool IsMember(const TNode i) const throw(ERRange)
        {return (const_cast<abstractDiGraph&>(G).Divergence(i)-G.Demand(i) > G.delta);};
...
public:
    ...
    deficientNodes(const abstractDiGraph& _G) throw() :
        managedObject(_G.Context()),indexSet<TNode>(_G.N(),_G.Context()),G(_G) {};
