...
protected:
    ...
    TCap  delta; ///< Threshold for capacity scaling methods

    TNode  sourceNode; ///< Default source node
    TNode  targetNode; ///< Default target node
    TNode  rootNode;   ///< Default root node
...
public:
    ...
    /// \brief  Merge another graph into this object
    ///
    ...
    void  AddGraph(abstractMixedGraph& G) throw(ERRejected);
    ...
    ///
    /// \return  The number of display nodes
    ...
    virtual TNode  Source() const throw();

    /// \brief  Retrieve the default source node
    ///
    /// \return  The index of the default source node
    virtual TNode  Target() const throw();

    /// \brief  Retrieve the default source node
    ///
    /// \return  The index of the default source node
    TNode  Root() const throw();
...
public:
    ...
    /// \brief  Retrieve the label display anchor point of a giiven arc
    ///
    /// \param a      An arc index ranged [0,1,..,2m-1]
    ...
    /// \brief  Retrieve the port node or the first bend node of a given arc
    ///
    /// \param a  An arc index ranged [0,1,..,2m-1]
    /// \return   A bend node index ranged [n,n+1,..,n+ni-1]
    ///
    /// This respects the arc orientations. That is, for an odd index a,
    /// the final bend node is returned instead of the first bend node.
    TNode  PortNode(TArc a) const throw(ERRange);

    /// \brief  Get the sequence of bend nodes of a given arc
    ///
    /// \param a           An arc index ranged [0,1,..,2m-1]
    /// \param layoutNode  An array to be filled withthe bend nodes
    /// \param length      The dimension of layoutNode[]
    /// \return            The number of bend nodes incident with a
    ///
    /// This respects the arc orientations. That is, for an odd index a,
    /// the list starts with the final bend node instead of the first bend node.
...
public:
    ...
    TIndex  OriginalNode(TNode v) const throw(ERRange);
    ...
    /// OriginalArc(a)==OriginalArc(a^1).
    TIndex  OriginalArc(TArc a) const throw(ERRange);
    ...
    /// \addtogroup groupLayoutBasic
    /// @{

public:

    /// \brief  Fit the current XY-drawing into the first quadrant
    ///
    ...
    /// \brief  Fit the current XY-drawing into a given rectangle
    ///
    /// \param xMin  The minimum X-value in the new drawing
    /// \param xMax  The maximum X-value in the new drawing
    /// \param yMin  The minimum Y-value in the new drawing
    /// \param yMax  The maximum Y-value in the new drawing
    void  Layout_ScaleEmbedding(TFloat xMin,TFloat xMax,TFloat yMin,TFloat yMax) throw(ERRejected);
...
public:
    ...
    ///   are eliminated.
    /// - If the new model does not require differently, Layout_ArcAlignment()
...
public:
    ...
    /// spacing or, if spacing==0, the nodeSep configuration parameter value.
    ///
    ...
    /// - The node colours
    /// - The node indices
    void  Layout_Circular(int spacing=0) throw(ERRejected);
    ...
    void  Layout_CircularByPredecessors(int spacing=0) throw(ERRejected);
    ...
    void  Layout_CircularByColours(int spacing=0) throw(ERRejected);
    ...
    /// 
    bool  Layout_Outerplanar(int spacing=0) throw(ERRejected);
    ...
    /// becomes spacing or, if spacing==0, the nodeSep configuration parameter
    /// value.
    ///
    /// An outerplanar representation must be set in advance. If so, the procedure
    /// displays precisely the given combinatorial embedding:
    ...
    /// spanning tree.
    void  Layout_Equilateral(int spacing=0) throw(ERRejected);
...
private:
    ...
    void  Layout_AssignCircularCoordinates(int spacing=0,TNode* index=NULL) throw(ERRejected);
...
private:
    ...
    ///
    /// \param bendSep     The minimum distance between two parallel edge segments
    /// \param alignPorts  If true, all bend nodes are aligned like port nodes
    ///
    ...
    /// edges.
    void  Layout_Visibility2Connected(int bendSep = 0,bool alignPorts = true) throw(ERRejected);
...
public:
    ...
    void  Layout_KandinskyTree(TNode root = NoNode,int spacing = 0) throw(ERRejected);
    ...
    bool  Layout_HorizontalVerticalTree(TNode root = NoNode,int spacing = 0) throw(ERRejected);
...
public:
    ...
    void  Layout_ForceDirected(TOptFDP method=FDP_DEFAULT,int spacing=0) throw(ERRejected);
...
private:
    ...
    void  Layout_SpringEmbedder(int spacing=0) throw(ERRejected);
    ...
    void  Layout_GEMDrawing(TOptFDP method=FDP_GEM,int spacing=0) throw(ERRejected);
    ...
    void  Layout_LayeredFDP(TOptFDP method=FDP_LAYERED,int spacing=0) throw(ERRejected);
...
public:
    ...
    void  Layout_PredecessorTree(TOptAlign mode=ALIGN_OPTIMIZE,int dx=0,int dy=0) throw(ERRejected);
    ...
    /// grid line form independent sets, edges have one bend node on every
    ...
    void  Layout_Layered(int method=LAYERED_DEFAULT,int dx=0,int dy=0) throw(ERRejected);
...
private:
    ...
    bool  Layout_EdgeSpanMinimalLayering(int dy,TNode* nodeColour=NULL) throw();
    ...
    void  Layout_LayeringByColours(int dx,int dy) throw(ERRejected);
    ...
    /// critical.
    void  Layout_SweepLayerByLayer(int dx,int dy) throw(ERRejected);
    ...
    void  Layout_SetHorizontalCoordinates(int dx) throw();
...
public:
    ...
    /// this method generates a shortest path between two nodes or a tree rooted
    /// at a given node.
    bool  ShortestPath(TMethSPX method,TOptSPX characteristic,
    ...
    TNode  NegativeCycle(TOptSPX characteristic,TNode source=NoNode,TFloat epsilon=0) throw(ERRange);

    /// \brief  Perform a breadth first search
    ///
    /// \param characteristic  Either SPX_SUBGRAPH, SPX_RESIDUAL or SPX_PLAIN
    /// \param s               The node where the search tree is rooted at
    /// \param t               An optional target node
    /// \retval true           The target node could be reached
    ///
    /// This determines a predecessor tree where every node is reached from s
    /// on a path with a minimum number of arcs. If a target node t is passed,
    /// the procedure stops prematurely when t is reached.
    bool  BFS(TOptSPX characteristic,TNode s,TNode t=NoNode) throw(ERRange);

protected:

    /// \brief  Arc qualification for graph search algorithms
    ///
    /// \param characteristic  Either #SPX_PLAIN, #SPX_SUBGRAPH or #SPX_RESIDUAL
    /// \param a               The index of the considered arc ranged [0,1,..,2m-1]
    /// \retval true           The arc is traversable
    ///
    /// This tells which arcs may be traversed by graph search procedures like
    /// SPX_Dijkstra(). In case of #SPX_PLAIN, arcs must be non-blocking. That is,
    /// undirected edges can be always traversed, and directed arcs can be
    /// traversed in forward direction only. In case of #SPX_SUBGRAPH, the search
    /// is restricted to arcs a with Sub(a)>0. The #SPX_RESIDUAL option is used
    /// by network flow-codes to search the so-called residual graph.
    bool  Eligible(TOptSPX characteristic,TArc a) throw(ERRange);
    ...
    TNode  SPX_Dijkstra(TOptSPX characteristic,const indexSet<TNode>& S,const indexSet<TNode>& T)
    ...
    bool  SPX_FIFOLabelCorrecting(TOptSPX characteristic,TNode s,TNode t=NoNode)
    ...
    bool  SPX_BellmanFord(TOptSPX characteristic,TNode s,TNode t=NoNode)
                throw(ERRange,ERCheck);
...
public:
    ...
    /// 3, depending on the implicit orientation.
    bool  ImplicitSTOrientation(TNode s=NoNode, TNode t=NoNode) throw();
...
public:
    ...
    bool  EulerCycle(TArc* pred = NULL) throw(ERRejected);
...
protected:
    ...
    virtual TFloat  MXC_Heuristic(TMethMaxCut method,TNode s = NoNode,TNode t = NoNode)
                        throw(ERRange,ERRejected);
...
public:
    ...
    TFloat  CutCapacity(TFloat separator=InfFloat) throw(ERCheck);
...
inline TNode abstractMixedGraph::Source() const throw()
{
    return sourceNode;
}


inline TNode abstractMixedGraph::Target() const throw()
{
    return targetNode;
}


inline TNode abstractMixedGraph::Root() const throw()
{
    return rootNode;
}
